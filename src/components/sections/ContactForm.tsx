...
import { EMAIL, WHATSAPP_NUMBER, LOCATIONS } from "@/lib/constants";
...
function buildWhatsAppUrl(data: FormData): string {
  const lines = [
    ...
    `\nMe gustaria solicitar una consulta sobre cirugía bariátrica.`,
  ];
    ...
    <SectionWrapper id="contacto" bg="white">
      <div className="text-center">
        ...
        <p className="mx-auto mt-4 max-w-2xl text-lg text-secondary-500">
          ...
          información pre-cargada. Respondemos en menos de 2 horas.
        </p>
      </div>
      ...
      <div className="mt-12 grid gap-12 lg:grid-cols-2">
        ...
        <div className="rounded-2xl bg-gray-50 p-5 sm:bg-transparent sm:p-0 sm:rounded-none">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5" noValidate>
            ...
            <Button type="submit" variant="whatsapp" className="w-full sm:w-auto text-base">
              ...
              Enviar Consulta Gratuita por WhatsApp
            </Button>
          </form>
        </div>
        ...
        <div className="space-y-6">
          <div className="space-y-5">
            {LOCATIONS.map((loc) => (
              <div key={loc.city} className="rounded-2xl border border-gray-200 p-5">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-secondary-700">{loc.city}</p>
                    <a
                      href={`https://wa.me/${loc.whatsapp}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-2 mt-2 text-sm text-[#25D366] font-semibold hover:underline"
                    >
          ...
          <div className="rounded-2xl bg-primary-50 p-6 text-center">
            <p className="text-sm text-secondary-600 leading-relaxed">
              En Grupo ByM te acompañamos en cada etapa: antes, durante y después
              de la cirugía. Tu mejor elección.
            </p>
          </div>
        </div>
      </div>
